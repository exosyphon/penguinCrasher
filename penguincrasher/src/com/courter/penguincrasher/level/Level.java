...
package com.courter.penguincrasher.level;
...
import com.courter.penguincrasher.entity.Door;
import com.courter.penguincrasher.entity.Entity;
import com.courter.penguincrasher.entity.Gore;
...
public class Level {
	...
	private int tick;
	private int calculatedYO;

	@SuppressWarnings("unchecked")
	public Level (GameScreen screen, int w, int h, int xo, int yo, int xSpawn, int ySpawn, int worldNum) {
		this.screen = screen;
		this.xSpawn = xSpawn;
		this.ySpawn = ySpawn;
		
		this.calculatedYO = h * (5 * (worldNum - 1)); 
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				int col = (Art.levels.getPixel(x + xo * 31, y + yo * 12 + calculatedYO) & 0xffffff00) >>> 8;
				byte wall = 0;
				...
				if (col == 0xffffff)
					...
					add(new Door(x * 10, y * 10, col >> 16 & 0xff));
				}
				walls[x + y * w] = wall;
			}
		}
	...
	public void render (Screen screen, Camera camera) {
		...
		int xo = 0;
		int yo = 0;
		for (int x = xo; x <= xo + 240; x++) {
			for (int y = yo; y <= yo + 320; y++) {
				if (x >= 0 && y >= 0 && x < width && y < height) {
					int ximg = 0;
					int yimg = 0;
